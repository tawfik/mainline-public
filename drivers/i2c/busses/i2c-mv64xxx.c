...
mv64xxx_i2c_offload_xfer(struct mv64xxx_i2c_data *drv_data)
{
	...
	if (num == 1 && !(msgs[0].flags & I2C_M_RD)) {
		ctrl_reg |= MV64XXX_I2C_BRIDGE_CONTROL_WR |
			(msgs[0].len - 1) << MV64XXX_I2C_BRIDGE_CONTROL_TX_SIZE_SHIFT;
		mv64xxx_i2c_prepare_tx(drv_data);
	}
	/* Single read message transaction */
	else if (num == 1 && msgs[0].flags & I2C_M_RD) {
		ctrl_reg |= MV64XXX_I2C_BRIDGE_CONTROL_RD |
			(msgs[0].len - 1) << MV64XXX_I2C_BRIDGE_CONTROL_RX_SIZE_SHIFT;
	}
	/*
	 ...
	 */
		...
		ctrl_reg |=
			...
			(msgs[0].len - 1) << MV64XXX_I2C_BRIDGE_CONTROL_TX_SIZE_SHIFT |
			(msgs[1].len - 1) << MV64XXX_I2C_BRIDGE_CONTROL_RX_SIZE_SHIFT |
